...
import datetime
import requests
import operator # Needed to sort the csv.reader data

# Placed the FILE_URL variable into global scope.
...
# I removed the data variable from the local scope and am 
# placing it here in the global scope.
data = get_file_lines(FILE_URL)


def get_same_or_newer(start_date):
    """Returns the employees that started on the given date, or the closest one."""
    reader_unsorted = csv.reader(data[1:])
    reader = sorted(reader_unsorted, key=operator.itemgetter(3))
